...
describes what users get. Pick a format with `-o`, or let artic choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
artic <command> -o table   # aligned columns for reading
artic <command> -o jsonl   # one JSON object per line, for piping
artic <command> -o json    # a single JSON array
artic <command> -o csv     # spreadsheet friendly
artic <command> -o tsv     # tab-separated
artic <command> -o url     # just the URL column
artic <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
artic <command> --fields id,title,url
```
...
```bash
artic <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
artic <command>            # a table, because this is a terminal
artic <command> | wc -l    # JSONL, because this is a pipe
```
